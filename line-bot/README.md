...
LINE App                          手機的行事曆 App
   │  訊息 / 按鈕                      │  訂閱 .ics（每小時更新）
   ▼                                  ▼
LINE Platform ──webhook──▶ Cloudflare Worker ──▶ KV（待辦、使用者、家事、討論）
   ▲                            │
   └────── reply / push ────────┘
                                │  「一週」附連結
                                ▼
...
| `一週` | 七天輪播卡 ＋ 完整網頁連結 |
| `分工` | 今晚誰做 A（家裡）、誰做 B（小孩）；`換班`把順序翻過來 |
| `家事` | 今天白天洗什麼、長週期進度（逾期標紅，每項都有「做了」按鈕） |
| `家事 冷氣濾網` | 記成今天做過 |
| `家事 床單 8/20` | 補以前做的日期（`2026-08-20` 也可以） |
| `洗陽台做了` | 同上，認不出是哪一項就當成一般待辦 |
| `行事曆` | 接下來兩週有排時間的事，照日期分組；過期沒處理的另外標紅 |
| `紀錄` | 最近三週做過、決定過什麼——完成的待辦、記下的家事、討論的答案 |
| `一句` | 抽一則小提示（育兒／家務／提醒／這個家的數字）；`一句 關`停掉每天 11:15 那則 |
| `興趣` | 從八個面向隨機抽一項；`興趣 清單` 看哪一項最久沒碰、`興趣 做了 攝影` 記一筆 |
| `行事曆連結` | 拿到 `.ics` 訂閱網址，加到 iPhone／Google 日曆；`行事曆連結 重設`換一組 |
...
- **每天 11:15** — 一句話（育兒／家務／提醒／這個家的數字）＋今天白天要洗的種類。
  挑這個時間是因為媽媽從 08:30 起單獨帶妹妹，到這時是第三個小時，疲勞感開始出現、午餐還沒著落
- **週五的 11:15 那則**後面接這週的討論清單（不另外發，省一則）
- **每天 19:30** — 今晚的 A／B 分工（洗澡開始分頭進行的那一刻）、明天換誰、逾期的長週期家事、未完成待辦
- **週三、週日 23:00** — 八種興趣裡抽一項（身體／大腦／創造／內在／連結／收入／冒險／好玩）
...
晚上的 A／B 分工在 [`src/data.js`](src/data.js) 的 `EVENING`，輪到誰由
[`src/evening.js`](src/evening.js) 按日期算，`換班` 只是把順序翻一位。

11:15 那句話在 [`src/notes.js`](src/notes.js)。有兩種：寫死的句子，
以及用這個家的資料算出來的（小孩今天幾個月、這週清掉幾件待辦、下次洗床單還有幾天）。
算不出來的會自動跳過換下一則，所以不會出現空句子。
...
- 行事曆訂閱網址帶一串隨機 token，**網址本身就是密碼**——拿到的人就讀得到，
  所以別轉貼；要換用「行事曆連結 重設」，舊網址立刻失效。
